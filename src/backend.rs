...
impl CommitNode {
    pub fn create(commit: git2::Commit, commits: &mut HashMap<String, CommitNode>) -> String {
        if commits.contains_key(&commit.id().to_string()) {
            commit.id().to_string()
        } else {
            let mut result = CommitNode { id: commit.id().to_string(), parents: Vec::new(), children: Vec::new() };
...
pub fn get_commit_height(commit: &CommitNode, commits: &HashMap<String, CommitNode>) -> isize {
    ...
    if commit.parents.len() == 0 {
        ...
        if parent.children.len() == 1 {
            ...
            get_commit_height(parent, commits) + if parent.children.iter().position(|c| c == &commit.id).unwrap() == 0 { -1 } else { 1 }
        }
    }
}
